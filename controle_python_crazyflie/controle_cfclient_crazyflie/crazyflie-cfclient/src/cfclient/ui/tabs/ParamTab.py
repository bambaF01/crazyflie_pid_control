...
try:
    ...
    HAVE_PARAM_FILE_MANAGER = True
except ImportError:
    ParamFileManager = None
    HAVE_PARAM_FILE_MANAGER = False
...
class ParamTab(TabToolbox, param_tab_class):
    ...
    def _load_param_button_clicked(self):
        if not HAVE_PARAM_FILE_MANAGER:
            QMessageBox.about(self, 'Warning', 'Param file support is not available. Please update cflib.')
            return
