...
activate_Optitrack = False

class MocapWrapper(Thread):
    def __init__(self, natnetparams:NatNetParams, rb_id:int):
        Thread.__init__(self)

        self.rigidbody_id = rb_id
        self.on_pose = None
        self._stay_open = True

        self.client = NatNetClient(natnetparams)
    ...
    def run(self):
        ...
        mocap_generator = self.client.MoCap() # generator of frames
        while self._stay_open:
            for frame_data in mocap_generator:
                rb = frame_data.rigid_body_data.rigid_bodies_d
                if self.rigidbody_id in rb and self.on_pose:
                        rb = rb[self.rigidbody_id]
                        self.on_pose(rb.pos, rb.rot)
            time.sleep(0.1)
...
class FlightTab(TabToolbox, flight_tab_class):
    ...
    def __init__(self, helper):
        ...
        self.targetTakeoffHeight.valueChanged.connect(self._takeoff_height_changed)
        ...
        self._helper.inputDeviceReader.sequence_updated.add_callback(self.sequence_updated)
        self._tf_state = 0
        self._ring_effect = 0
        ...
        if activate_Optitrack and HAVE_NATNET:
            # === Configuration ===
            SERVER_IP = "192.168.2.105"       # Motive PC IP
            LOCAL_IP = "192.168.2.102"         # Local machine IP
            COMMAND_PORT = 1510
            DATA_PORT = 1511
            MULTICAST = True
            MULTICAST_ADDRESS = "239.255.42.99"  # Default for NatNet (only used if MULTICAST=True)

            # Build parameter object once
            self.NATNET_PARAMS = NatNetParams(
            ...
            self.optitrack = MocapWrapper(self.NATNET_PARAMS, self.rigid_body_id)

        self.takeoff_height = 0.4
    
    def send_extpose(self, posi, quat):
        """
        ...
        # [x, y, z], [qx, qy, qz, qw]
        # frame : https://forum.bitcraze.io/viewtopic.php?t=4385 (z up)
        self._helper.cf.extpos.send_extpose(posi.z, posi.x, posi.y, quat.z, quat.x, quat.y, quat.w)
        # self._helper.cf.extpos.send_extpos(posi.z, posi.x, posi.y)

    def adjust_orientation_sensitivity(self):
        self._helper.cf.param.set_value('locSrv.extQuatStdDev', 8.0e-3)
    
    def activate_kalman_estimator(self):
        self._helper.cf.param.set_value('stabilizer.estimator', '2')

        # Set the std deviation for the quaternion data pushed into the
        # kalman filter. The default value seems to be a bit too low.
        self._helper.cf.param.set_value('locSrv.extQuatStdDev', 0.06)
    ...
    def _pose_data_received(self, pose_logger, pose):
        if self.isVisible():
            ...
            self.estimateX.setText(("%.3f" % pose[0]))
            self.estimateY.setText(("%.3f" % pose[1]))
            self.estimateZ.setText(("%.3f" % estimated_z))
            self.estimateRoll.setText(("%.3f" % roll))
            self.estimatePitch.setText(("%.3f" % pitch))
            self.estimateYaw.setText(("%.3f" % pose[5]))
    ...
    def connected(self, linkURI):
        ...
        if activate_Optitrack and HAVE_NATNET:
            self.optitrack.on_pose = lambda position, orientation: self.send_extpose(position, orientation)
    ...
    def _takeoff_height_changed(self, value):
        logger.debug("Takeoff height updated to [%f] m" % value)
        self.takeoff_height = value
        Config().set("takeoff_height", value)
    ...
    def alt1_updated(self, data):
        print('Takeoff... height=0.3')
        self._helper.cf.high_level_commander.takeoff(0.3, 1, yaw=None)
        # time.sleep(0.5)
        # self._helper.cf.high_level_commander.go_to(0,0,self.height_safe,0,2)
        # time.sleep(70)
        # for k in range(3):
        #     print(f"Going down to %f"%(self.height_safe-(k+1)*0.025))
        #     self._helper.cf.high_level_commander.go_to(0,0,-0.025,0,2,True, True)
        #     time.sleep(70)
        # print('\nend\n')
        # time.sleep(20)
    ...
    def alt2_updated(self, data):
        print('Landing...')
        ISFLYING.STATUS = 0
        self._helper.cf.high_level_commander.land(0,2)

    def sequence_updated(self, data):
        if not data or self._sequence_running:
            return

        self._sequence_running = True
        Thread(target=self._run_sequence_triangle, daemon=True).start()

    def _run_sequence_triangle(self):
        try:
            current_z = self._helper.pose_logger.position[2]
        except Exception:
            print("[SEQ] Position non disponible")
            self._sequence_running = False
            return

        if abs(current_z - 0.3) > 0.1:
            print("[SEQ] Sequence refusee: pas en stationnaire a 0.3 m")
            self._sequence_running = False
            return
