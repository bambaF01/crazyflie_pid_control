...
from .tabs.FlightTab import ISFLYING
...
class MainUI(QtWidgets.QMainWindow, main_window_class):
    ...
    def __init__(self, *args):
        ...
        self._battery_info_label = QLabel("")
        self.statusBar().addPermanentWidget(self._battery_info_label)
        ...
        self.joystickReader.input_updated_2.add_callback(
            lambda *args: self._disable_input or
            self.new_high_level_commander(*args))
        ...
        if hasattr(self.cf, 'link_statistics') and \
                hasattr(self.cf.link_statistics, 'link_quality_updated'):
            ...
            logger.warning('Link statistics not available, disabling link quality updates')
    ...
    def new_high_level_commander(self,vx,vy,vz,yaw):
        duration_s = 0.05
        if ISFLYING.STATUS:
            if (vx,vy,vz,yaw) != (0.0,0.0,0.0,0.0):
                ...
                print(f"relative: x={vx}, y={vy}, z={vz}, yaw={yaw}")
                self.cf.high_level_commander.go_to(vx, vy, vz, yaw, duration_s,relative=True)
    ...
    def _update_battery(self, timestamp, data, logconf):
        ...
        self._last_vbat = data["pm.vbat"]

    def _update_battery_ext(self, timestamp, data, logconf):
        current_a = None
        power_w = None
        if "pm.ibat" in data:
            current_a = abs(float(data["pm.ibat"]))
        elif "pm.current" in data:
            current_a = abs(float(data["pm.current"]))
        elif "pm.ibat" in data:
            current_a = abs(float(data["pm.ibat"]))

        if "pm.power" in data:
            power_w = abs(float(data["pm.power"]))
        elif "pm.powerfilt" in data:
            power_w = abs(float(data["pm.powerfilt"]))
        elif current_a is not None and self._last_vbat is not None:
            power_w = current_a * float(self._last_vbat)

        txt_parts = []
        if current_a is not None:
            txt_parts.append(f"{current_a*1000:.0f} mA")
        if power_w is not None:
            txt_parts.append(f"{power_w:.2f} W")
        if txt_parts:
            self._battery_info_label.setText(" | ".join(txt_parts))
        else:
            self._battery_info_label.setText("")

    def _update_power_estimate(self, timestamp, data, logconf):
        thrust = float(data.get("stabilizer.thrust", 0.0))
        thr_norm = thrust / 65535.0
        if thr_norm < 0.0:
            thr_norm = 0.0
        if thr_norm > 1.0:
            thr_norm = 1.0
        ...
        K_A = 5.0
        ALPHA = 2.2
        ibat_est = max(0.0, I_IDLE_A + K_A * (thr_norm ** ALPHA))
        vbat = self._last_vbat
        if vbat is not None:
            power_w = ibat_est * float(vbat)
            print(f"[PowerEst] thrust={thr_norm:.2f} I≈{ibat_est*1000:.0f} mA, P≈{power_w:.2f} W, V={float(vbat):.2f} V")
        else:
            print(f"[PowerEst] thrust={thr_norm:.2f} I≈{ibat_est*1000:.0f} mA, V=--")
    ...
    def _connected(self):
        ...
        try:
            lg_ext = LogConfig("BatteryExt", 500)
            candidates = [
                "pm.ibat",
                "pm.current",
                "pm.ibat",
            ...
            for name in candidates:
                if self.cf.log.toc.get_element_by_complete_name(name):
                    lg_ext.add_variable(name)
                    added = True
            if added:
                self.cf.log.add_config(lg_ext)
                lg_ext.data_received_cb.add_callback(self._update_battery_ext)
                lg_ext.error_cb.add_callback(self._log_error_signal.emit)
                lg_ext.start()
        except Exception as e:
            logger.warning(str(e))

        try:
            lg_est = LogConfig("PowerEstimate", 500)
            if self.cf.log.toc.get_element_by_complete_name("stabilizer.thrust"):
                lg_est.add_variable("stabilizer.thrust", "uint16_t")
                self.cf.log.add_config(lg_est)
                lg_est.data_received_cb.add_callback(self._update_power_estimate)
                lg_est.error_cb.add_callback(self._log_error_signal.emit)
                lg_est.start()
        except Exception as e:
            logger.warning(str(e))
